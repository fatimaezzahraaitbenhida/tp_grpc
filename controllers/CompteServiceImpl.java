...
public class CompteServiceImpl extends CompteServiceGrpc.CompteServiceImplBase {
    ...
    public void totalSolde(GetTotalSoldeRequest request, StreamObserver<GetTotalSoldeResponse> responseObserver) {
        // Récupération de tous les comptes via le service
        var comptes = compteService.findAllComptes();

        // Calcul des statistiques : total, nombre, et moyenne
        int count = comptes.size();
        double sum = comptes.stream()
                .mapToDouble(ma.projet.grpc.entities.Compte::getSolde)
                .sum();
        double average = count > 0 ? sum / count : 0;

        // Construction de l'objet SoldeStats
